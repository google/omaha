...
HRESULT ProcessOneClickResponseXml(const CString& url_domain_requested,
  ...
  const TCHAR* const kOneClickHostCheckStatusPass = _T("pass");

  response_xml.Trim();

  if (response_xml.IsEmpty()) {
    return E_INVALIDARG;
  }

  CComPtr<IXMLDOMDocument> xml_document;
  HRESULT hr = LoadXMLFromMemory(response_xml, false, &xml_document);
  if (FAILED(hr)) {
    CORE_LOG(LE, (_T("[ProcessOneClickResponseXml LoadXml Failed][0x%08x]"),
                  hr));
    return E_INVALIDARG;
  }

  // Note:  We can't use "if (FAILED(hr))" below since MSXML returns S_FALSE in
  // many of these parsing cases.

  CComPtr<IXMLDOMNode> hostcheck_node;
  hr = xml_document->selectSingleNode(CComBSTR(L"/OneClick/HostCheck"),
                                      &hostcheck_node);
  if (S_OK != hr) {
    CORE_LOG(LE, (_T("[ProcessOneClickResponseXml Can't find ")
                  _T(" /OneClick/HostCheck node][0x%08x]"), hr));
    return E_INVALIDARG;
  }

  CComPtr<IXMLDOMNamedNodeMap> attributes;
  hr = hostcheck_node->get_attributes(&attributes);
  if (S_OK != hr) {
    CORE_LOG(LE, (_T("[ProcessOneClickResponseXml][No attributes][0x%08x]"),
                  hr));
    return E_INVALIDARG;
  }

  CComPtr<IXMLDOMNode> status_node;
  hr = attributes->getNamedItem(CComBSTR(L"status"), &status_node);
  if (S_OK != hr) {
    CORE_LOG(LE, (_T("[ProcessOneClickResponseXml]")
                  _T("[Failed getting status node][0x%08x]"), hr));
    return E_INVALIDARG;
  }

  CComBSTR status_text_bstr;
  hr = status_node->get_text(&status_text_bstr);
  if (S_OK != hr) {
    CORE_LOG(LE, (_T("[ProcessOneClickResponseXml]")
                  _T("[Failed getting status text][0x%08x]"), hr));
    return E_INVALIDARG;
  }

  CComPtr<IXMLDOMNode> domain_node;
  hr = attributes->getNamedItem(CComBSTR(L"domain"), &domain_node);
  if (S_OK != hr) {
    CORE_LOG(LE, (_T("[ProcessOneClickResponseXml]")
                  _T("[Failed getting domain node][0x%08x]"), hr));
    return E_INVALIDARG;
  }

  CComBSTR domain_text_bstr;
  hr = domain_node->get_text(&domain_text_bstr);
  if (S_OK != hr) {
    CORE_LOG(LE, (_T("[ProcessOneClickResponseXml]")
                  _T("[Failed getting domain text][0x%08x]"), hr));
    return E_INVALIDARG;
  }

  CString status_text = CString(status_text_bstr);
  CString domain_text = CString(domain_text_bstr);

  if ((status_text.CompareNoCase(kOneClickHostCheckStatusPass) == 0) &&
      (domain_text.CompareNoCase(url_domain_requested) == 0)) {
    // We have a match for "pass" and a match for the domain.
    return S_OK;
  } else {
    CORE_LOG(LE, (_T("[ProcessOneClickResponseXml status/domain dont match]")));
    return GOOPDATE_E_ONECLICK_HOSTCHECK_FAILED;
  }
}

// Calls a web service to validate the domain URL that the plugin called us
// from.  If we get a HTTP_STATUS_OK back we're good, otherwise we fail.
HRESULT VerifyParamsViaWebService(CString plugin_check_url,
                                  const CommandLineArgs& args) {
  CORE_LOG(L2, (_T("[VerifyParamsViaWebService]")));

  CString request_string;
  HRESULT hr = BuildOneClickRequestString(args, &request_string);
  if (FAILED(hr)) {
    CORE_LOG(L2, (_T("[BuildOneClickRequestString failed][0x%08x]"), hr));
    return hr;
  }

  plugin_check_url.Append(request_string);

  // Send the request.
  CORE_LOG(L2, (_T("[VerifyParamsViaWebService][url = %s]"), plugin_check_url));
  ...
  // Do not use CUP if the url contains a trusted tester token. The tttoken
  // should not be sent out in the clear in production.
  bool is_trusted_tester = String_Contains(args.webplugin_args, _T("tttoken="));
  if (!is_trusted_tester) {
    network_request.AddHttpRequest(new CupRequest(new SimpleRequest));
  }
  network_request.AddHttpRequest(new SimpleRequest);
  if (!is_trusted_tester) {
    network_request.AddHttpRequest(new CupRequest(new BrowserRequest));
  }

  CString response;
  hr = GetRequest(&network_request, plugin_check_url, &response);
  if (FAILED(hr)) {
    CORE_LOG(L2, (_T("[VerifyParamsViaWebService GET failed][0x%08x]"), hr));
    return hr;
  }

  int status_code = network_request.http_status_code();
  CORE_LOG(L1, (_T("[webplugincheck server returned %d]"), status_code));

  if (status_code == HTTP_STATUS_OK) {
    return ProcessOneClickResponseXml(args.webplugin_urldomain, response);
  } else {
    CORE_LOG(L2, (_T("[VerifyParamsViaWebService bad status code][%d]"),
                  status_code));
    return GOOPDATE_E_ONECLICK_HOSTCHECK_FAILED;
  }
}
