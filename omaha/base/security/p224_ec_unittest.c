...
#define kNumNISTTestVectors  52

// kNISTTestVectors are the NIST test vectors for P224.
static const struct TestVector kNISTTestVectors[kNumNISTTestVectors] = {
  {
...
void testScalarBaseMult() {
  p224_point point;
  int i;

  for (i = 0; i < kNumNISTTestVectors; i++) {
    uint8_t bin[56];
    p224_base_point_mul(kNISTTestVectors[i].scalar, &point);
    p224_point_to_bin(&point, bin);
    if (memcmp(bin, kNISTTestVectors[i].affine, 56) != 0) {
      printf("testScalarBaseMult fail!\n");
      exit(1);
    }
  }
}
...
void testScalarMult() {
  p224_point point;
  p224_point basepoint;
  int i;

  p224_point_from_bin(kBasePointExternal, sizeof(kBasePointExternal), &basepoint);

  for (i = 0; i < kNumNISTTestVectors; i++) {
    uint8_t bin[56];
    p224_point_mul(&basepoint, kNISTTestVectors[i].scalar, &point);
    p224_point_to_bin(&point, bin);
    if (memcmp(bin, kNISTTestVectors[i].affine, 56) != 0) {
      printf("testScalarMult fail!\n");
      exit(1);
    }
  }
}
