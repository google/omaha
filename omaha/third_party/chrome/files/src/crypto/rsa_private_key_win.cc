...
bool PrivateKeyInfoCodec::Import(const std::vector<uint8>& input) {
  ...
  size_t mod_size = modulus_.size();
  ASSERT1(mod_size % 2 == 0);
  size_t primes_size = mod_size / 2;
...
bool PrivateKeyInfoCodec::ReadIntegerWithExpectedSize(uint8** pos,
  ...
  size_t pad = expected_size - temp.size();
  size_t index = 0;
  if (out->size() == expected_size + 1) {
    ASSERT1(out->front() == 0x00);
    pad++;
    index++;
  } else {
    ASSERT1(out->size() <= expected_size);
  }
...
bool PrivateKeyInfoCodec::ReadLength(uint8** pos, uint8* end, uint32* result) {
  ...
  if (!(**pos & 0x80)) {
    ...
    ASSERT1(length_of_length <= 4);
    (*pos)++;
    ASSERT1(*pos + length_of_length < end);
...
bool PrivateKeyInfoCodec::ReadTypeHeaderAndLength(uint8** pos,
                                                  ...
                                                  uint8 expected_tag,
                                                  uint32* length) {
  UNREFERENCED_PARAMETER(expected_tag);
  ASSERT1(*pos < end);
  ASSERT1(**pos == expected_tag);
  (*pos)++;
...
bool PrivateKeyInfoCodec::ReadAlgorithmIdentifier(uint8** pos, uint8* end) {
  UNREFERENCED_PARAMETER(end);
  ASSERT1(*pos + sizeof(kRsaAlgorithmIdentifier) < end);
  ASSERT1(memcmp(*pos, kRsaAlgorithmIdentifier,
