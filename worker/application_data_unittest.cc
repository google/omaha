...
void FillAppData(AppData* app_data) {
  GUID app_id = StringToGuid(kGuid1);
  bool is_machine_app = true;
  GUID parent_app_id = StringToGuid(kGuid2);
  CString version = _T("12345");
  CString previous_version = _T("11111");
  CString language = _T("en");
  AppData::ActiveStates did_run = AppData::ACTIVE_RUN;
  CString ap = _T("some_ap_value");
  GUID iid = StringToGuid(kGuid3);
  CString brand_code = _T("GOOG");
  CString client_id = _T("some_client_id");
  CString referral_id = _T("ABC987");
  bool is_oem_install = true;
  bool is_eula_accepted = false;
  CString encoded_installer_data = _T("%20foobar");
  CString install_data_index = _T("foobar");
  bool is_uninstalled = true;
...
TEST(AppDataTest, TestAllParams) {
  GUID app_guid = StringToGuid(kGuid1);
  bool is_machine_app = true;
  GUID parent_app_guid = StringToGuid(kGuid2);
  CString version = _T("12345");
  CString previous_version = _T("11111");
  CString language = _T("en");
  AppData::ActiveStates did_run = AppData::ACTIVE_RUN;
  CString ap = _T("some_ap_value");
  CString tt_token = _T("some_tt_token_value");
  GUID iid = StringToGuid(kGuid3);
  CString brand_code = _T("GOOG");
  CString client_id = _T("some_client_id");
  CString referral_id = _T("123456");
  bool is_oem_install = true;
  bool is_eula_accepted = false;
  CString encoded_installer_data = _T("%20foobar");
  CString install_data_index = _T("foobar");
